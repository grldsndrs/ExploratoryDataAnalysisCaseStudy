...
fSCC<-filter(.data = SCC,grepl(".*\\b[Cc]oal\\b.*",SCC$Short.Name) & grepl(".*\\b[Cc]omb\\b.*",SCC$EI.Sector))
gyear<-NEI[NEI$SCC %in% fSCC$SCC,]%>%group_by(Pollutant,year)
...
p<-ggplot(data=mr, aes(x=type, y=Emissions, shape=type,colour=year)) +
  ...
  scale_x_discrete(name="Coal combustion related sources (all county inclusive)") +
